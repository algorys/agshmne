...
import io.github.algorys.agshmne.events.quest.FetchStackQuest;
...
public class FetchQuestSample {
	...
	public static void main(String[] args) {
		...
		FetchStackQuest appleQ = new FetchStackQuest(pj, new StackableItem("Pomme", 1), 3);
		System.out.println(appleQ.getGoal());

		boolean win = appleQ.isWin(pj);
		System.out.println("Quête finie ? " + win);
		for (int i = 0; i < 3; i++) {
			pj.getInventory().addItem(new StackableItem("Pomme", 1));
		}
